...
d3.select('#OSx').selectAll('option').data(params).enter().append('option').text(function(d){ return d});
d3.select('#OSy').selectAll('option').data(params).enter().append('option').text(function(d){ return d});
d3.select('#OSx').selectAll('option').nodes()[1].selected = true;
...
loadData().then(data => {
    ...
    d3.select('#OSx').on('change', newX);
    d3.select('#OSy').on('change', newY);
    ...
    function updateChart(){
        ...
        svg.selectAll('circle').data(data).enter().append("circle").attr("cx", function(d){
            return OSx(+d[xParam][year]);
        }).attr("cy", function(d){
            return OSy(+d[yParam][year]);
        }).attr("r", function(d){
